...
let zonesRendered = new Set();

attracties.forEach(function(a) {
  const zone = a[4].zone || "";
  if (!zonesRendered.has(zone)) {
    container.innerHTML += `<h2 class="title-groep" id="${zone}">${zoneTitles[zone] || zone}</h2>`;
    zonesRendered.add(zone);
  }
    ...
    if (conditions.accompanied) {
      heightInfo += `
        <div class="info-height only-default">
          Accompagné : ${conditions.accompanied}
        </div>`;
    }
    if (conditions.solo) {
      heightInfo += `
        <div class="info-height">
          Seul : ${conditions.solo}
        </div>`;
    }
    if (conditions.note) {
