// ✅ COMPLETE JS FILE VOOR JUNGLE CITY ATTRACTIEPAGINA MET ZONE-TITELS, STICKY MENU, HOOFDETITELS EN ICON SUPPORT
      ...
      if (typeof conditions === "object") {
        if (conditions.accompanied || conditions.solo) {
          ...
          if (conditions.accompanied) {
            heightInfo += `<div><img src='/site/files/images/Icons/icon-accompanied.png' style='height:24px;' alt='accompagné' /><br><span>${conditions.accompanied}</span></div>`;
          }
          if (conditions.solo) {
            heightInfo += `<div><img src='/site/files/images/Icons/icon-solo.png' style='height:24px;' alt='seul' /><br><span>${conditions.solo}</span></div>`;
          }
          heightInfo += `</div>`;
        }
        if (conditions.note) {
