...
// 📦 Attractiegegevens (note: zone is now inside each)
const attracties = [
    ...
    items.forEach(a => {
      ...
      if (conditions.accompanied || conditions.solo) {
        ...
        if (conditions.accompanied) {
          heightInfo += `<div><img src="https://www.jungle-city.be/site/files/images/Icons/accompanied.png" alt="accompagné"/><br><span>${conditions.accompanied}</span></div>`;
        }
        if (conditions.solo) {
          heightInfo += `<div><img src="https://www.jungle-city.be/site/files/images/Icons/solo.png" alt="seul"/><br><span>${conditions.solo}</span></div>`;
        }
        heightInfo += `</div>`;
      }
