document.addEventListener("DOMContentLoaded", function () {
  ...
  function formatConditions(conditions) {
    let parts = [];
    if (conditions.accompanied) parts.push(`${taal === "fr" ? "accompagné" : "begeleid"}: ${conditions.accompanied}`);
    if (conditions.solo) parts.push(`${taal === "fr" ? "seul" : "solo"}: ${conditions.solo}`);
    if (conditions.note) parts.push(conditions.note);
    return parts.join(", ");
  }

  function renderCards() {
    const selected = [...filterContainer.querySelectorAll("input[type='checkbox']:checked")].map(cb => cb.value);
    container.innerHTML = "";

    attracties.forEach(([id, name, conditions]) => {
      const info = formatConditions(conditions);
      if (selected.length && !selected.some(s => info.includes(s))) return;
      ...
      card.innerHTML = `
        ...
        <div class="hover-info">${info}</div>
      `;

      card.addEventListener("click", () => {
        modalBody.innerHTML = `
          <h2>${name}</h2>
          ${info ? `<p><strong>${taal === "fr" ? "Conditions" : "Voorwaarden"}:</strong> ${info}</p>` : ""}
