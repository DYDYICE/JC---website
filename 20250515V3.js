...
// 📦 Attractiegegevens (zone inside each)
const attracties = [
    ...
    items.forEach(a => {
      ...
      if (conditions.accompanied || conditions.solo) {
        ...
        if (conditions.accompanied) {
          heightInfo += `<div><img src="${icons[taal].accompanied}" alt="accompanied"/><br><span>${conditions.accompanied}</span></div>`;
        }
        if (conditions.solo) {
          heightInfo += `<div><img src="${icons[taal].solo}" alt="solo"/><br><span>${conditions.solo}</span></div>`;
        }
        heightInfo += `</div>`;
      }
