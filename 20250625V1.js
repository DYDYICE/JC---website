...
function insertHeightIcons() {
  ...
  cards.forEach(card => {
    ...
    const accMatch = details.match(/begeleid[: ]*([0-9+–\-to ]+cm)/i);
    if (accMatch) {
      ...
      acc.title = "Begeleid = kind onder toezicht";
      wrapper.appendChild(acc);
      added = true;
    }

    const soloMatch = details.match(/alleen[: ]*([0-9+–\-to ]+cm)/i);
    if (soloMatch) {
      ...
      solo.title = "Alleen = zelfstandig toegang";
...
function setupHeightFilter() {
  ...
  selector.addEventListener('change', () => {
    ...
    document.querySelectorAll('.attraction-card').forEach(card => {
      ...
      const accRanges = [...details.matchAll(/begeleid[: ]*([0-9]{1,3})[^\d]+([0-9]{1,3})cm/g)];
      ...
      const soloRanges = [...details.matchAll(/alleen[: ]*([0-9]{1,3})[^\d]+([0-9]{1,3})cm/g)];
      ...
      const accPlus = details.match(/begeleid[: ]*\+([0-9]{1,3})cm/i);
      if (accPlus && height >= parseInt(accPlus[1])) show = true;

      const soloPlus = details.match(/alleen[: ]*\+([0-9]{1,3})cm/i);
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      selector.value = '';
      document.querySelectorAll('.attraction-card').forEach(card => {
        card.style.display = (card.dataset.visible === 'false') ? 'none' : '';
      });
    });
  }
}
